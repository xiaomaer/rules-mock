/*
    ## RegExp Handler(根据解析后的描述文件生成匹配字符串)
...
export default {
    // const ast = RegExpParser.parse(regexp.source)
    gen(node, result, cache) {
        cache = cache || {
            guid: 1
        }
        return this[node.type] ? this[node.type](node, result, cache) : this.token(node, result, cache);
    },
    token(node, result, cache) {
        switch (node.type) {
            ...
            case 'non-word-boundary': // TODO
                break
            case 'digit':
                return Mock.randomSelect(NUMBER.split(''));
            case 'non-digit':
                return Mock.randomSelect((LOWER + UPPER + OTHER).split(''));
            case 'form-feed':
                break
            case 'line-feed':
            ...
            case 'carriage-return':
                break
            case 'white-space':
                return Mock.randomSelect(SPACE.split(''));
            case 'non-white-space':
                return Mock.randomSelect((LOWER + UPPER + NUMBER).split(''));
            case 'tab':
                break
            case 'vertical-tab':
                break
            ...
            case 'non-word': // \W [^a-zA-Z0-9]
                return Mock.randomSelect(OTHER.replace('_', '').split(''));
            case 'null-character':
                break
        }
        return node.body || node.text
    ...
    */
    alternate(node, result, cache) {
        // node.left/right {}
        return this.gen(
            Mock.bool() ? node.left : node.right,
            result,
            cache
        )
    },
    ...
    */
    charset(node, result, cache) {
        ...
        // node.body []
        let literal = Mock.randomSelect(node.body)
    ...
    'invert-charset': function (node, result, cache) {
        ...
        return Mock.randomSelect(pool.split(''))
    },
    // 反向引用
